...
def home():
    return {
        ...
        "version": "2.0",
        "total_apis": len(UNIQUE_APIS),
        "auth": "Key required: shuvo",
        "endpoints": {
            "/bomb": "POST/GET with phone + key parameter",
...
def bomb():
    ...
    # ====== KEY AUTH ======
    if key != VALID_KEY:
        return jsonify({"status": "error", "message": "Invalid API key. Use key: shuvo"}), 401
